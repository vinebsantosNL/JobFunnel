...
function scrape(): ScrapedJob | null {
  try {
    const title =
      document.querySelector<HTMLElement>('h1[data-at="job-ad-title"]')?.innerText?.trim() ??
      document.querySelector<HTMLElement>('h1.js-job-ad-title')?.innerText?.trim() ??
      document.querySelector<HTMLElement>('h1[class*="title"]')?.innerText?.trim()

    const company =
      document.querySelector<HTMLElement>('[data-at="job-ad-company-name"]')?.innerText?.trim() ??
      document.querySelector<HTMLElement>('.js-company-name')?.innerText?.trim() ??
      document.querySelector<HTMLElement>('[class*="company"]')?.innerText?.trim()

    const location =
      document.querySelector<HTMLElement>('[data-at="job-ad-location"]')?.innerText?.trim() ??
      document.querySelector<HTMLElement>('.js-job-ad-location')?.innerText?.trim() ??
      null

    const salaryText =
      document.querySelector<HTMLElement>('[data-at="job-ad-salary"]')?.innerText?.trim() ??
      null

    const { salaryMin, salaryMax, currency } = parseSalary(salaryText)

    const descEl =
      document.querySelector<HTMLElement>('[data-at="job-ad-description"]') ??
      document.querySelector<HTMLElement>('.js-job-ad-description') ??
...
// StepStone uses dynamic content — use MutationObserver
